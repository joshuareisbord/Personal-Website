...
import { locationsSchema, searchCities, selectCity, type LocationData } from '../../src/lib/locations';

const data: LocationData = {
  regions: [{ code: 'ON', name: 'Ontario' }, { code: 'QC', name: 'Quebec' }, { code: 'NU', name: 'Nunavut' }],
  cities: [
...
test('selection pairs the exact city coordinates with its full location label', () => {
  assert.deepEqual(selectCity({ code: 'CA', name: 'Canada' }, data, data.cities[0]!), {
    ...
    place: { latitude: 45.5019, longitude: -73.5674, countryCode: 'CA', regionCode: 'QC', city: 'Montréal' },
  });
  assert.throws(() => locationsSchema.parse({ ...data, cities: [{ ...data.cities[0], latitude: 100 }] }));
