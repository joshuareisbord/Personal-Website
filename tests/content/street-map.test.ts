...
import { buildStreetRoutes, fromMapCenter, nextMarkerIndex, toMapPosition } from '../../src/lib/street-map-data';
import { buildWorkJourney, interpolateJourneyLeg, type Coordinates, type JourneyExperience } from '../../src/lib/work-journey';
...
function role(company: string, startDate: string, coordinates?: Coordinates): JourneyExperience {
  return { company, title: 'Engineer', startDate, endDate: null,
    ...(coordinates ? { place: { longitude: coordinates[0], latitude: coordinates[1], countryCode: 'US' } } : {}) };
}

test('street/globe coordinates round-trip with longitude first and wrap repeated worlds', () => {
  for (const coordinates of [[-122.4, 37.8], [179.9, -45], [0, 0]] satisfies Coordinates[]) {
...
test('great-circle routes stay ordered and unwrap both antimeridian directions', () => {
  for (const endpoints of [[[170, 20], [-170, 30]], [[-170, 30], [170, 20]]] satisfies [Coordinates, Coordinates][]) {
    const journey = buildWorkJourney([role('Earlier', '2020', endpoints[0]), role('Later', '2021', endpoints[1])]);
    ...
    for (let index = 1; index < route.length; index++) assert.ok(Math.abs(route[index]![1] - route[index - 1]![1]) < 180);
    const midpoint = interpolateJourneyLeg(journey.legs[0]!, 0.5);
    assert.ok(Math.abs(route[48]![0] - midpoint[1]) < 1e-10, 'Sample the existing great circle, not a straight road route.');
  }
});

test('unmapped roles stay unmapped and shared markers cycle every role', () => {
  const journey = buildWorkJourney([role('<img onerror=alert(1)>', '2020', [10, 20]), role('Shared', '2021', [10, 20]),
    { ...role('Remote', '2022'), location: 'London' }, role('Last', '2023', [40, 50])]);
...
test('SSR stays safe and a failed lazy import leaves retry and back available', async () => {
  ...
  const element = createElement(StreetMap, { journey: buildWorkJourney([]), selected: undefined,
    initialCenter: center, initialZoom: 7, onSelect: () => {}, onBack: (value) => returned.push(value) });
  ...
  try {
    ...
    const button = (label: string): HTMLButtonElement => [...host.querySelectorAll('button')].find((item) => item.textContent === label)!;
