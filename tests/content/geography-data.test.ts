...
test('committed geography includes usable world and regional outlines and every country lookup', async () => {
  ...
  const topology = JSON.parse(
    await readFile(new URL('boundaries.topo.json', root), 'utf8'),
  ) as Topology<Objects<Record<string, unknown>>>;
  ...
  const catalog = countriesSchema.parse(
    JSON.parse(await readFile(new URL('countries.json', root), 'utf8')),
  );
  assert.ok(catalog.length > 200);
  for (const country of catalog) {
    const data = locationsSchema.parse(
      JSON.parse(await readFile(new URL(`locations/${country.code}.json`, root), 'utf8')),
    );
    ...
    if (country.code === 'CA') {
      ...
      assert.ok(
        data.cities.some(
          (city) => city.name === 'Toronto' && city.longitude < -79 && city.latitude > 43,
        ),
      );
    }
    if (country.code === 'US') {
      ...
      assert.ok(
        data.cities.some(
          (city) => city.name === 'Los Angeles' && city.longitude < -118 && city.latitude > 34,
        ),
      );
    }
  }
