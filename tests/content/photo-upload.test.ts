...
test('photo input and prepared upload validation share strict type and byte limits', () => {
  for (const type of ['image/png', 'image/jpeg', 'image/webp'])
    validatePhotoInput({ type, size: MAX_PHOTO_INPUT_BYTES });
  for (const type of ['', 'image/gif', 'image/svg+xml', 'text/plain'])
    assert.throws(() => validatePhotoInput({ type, size: 10 }), /PNG, JPEG, or WebP/);
  for (const size of [0, -1, NaN, Infinity, MAX_PHOTO_INPUT_BYTES + 1])
    assert.throws(() => validatePhotoInput({ type: 'image/jpeg', size }));
  validatePreparedPhoto({ type: PHOTO_OUTPUT_TYPE, size: MAX_PHOTO_OUTPUT_BYTES });
  for (const size of [0, NaN, MAX_PHOTO_OUTPUT_BYTES + 1])
    assert.throws(() => validatePreparedPhoto({ type: PHOTO_OUTPUT_TYPE, size }));
...
test('photo dimensions preserve aspect ratio, avoid upscaling, and reject excessive decoded images', () => {
  ...
  for (const [width, height] of [
    ...
    assert.throws(() => getPhotoDimensions(width!, height!));
});
...
test('preparation rejects invalid input before decoding and reports corrupt image data', async () => {
  ...
  try {
    await assert.rejects(
      preparePhoto(new File(['x'], 'fake.svg', { type: 'image/svg+xml' })),
      /PNG, JPEG, or WebP/,
    );
    assert.equal(attempts, 0);
    await assert.rejects(
      preparePhoto(new File(['broken'], 'fake.jpg', { type: 'image/jpeg' })),
      /decode/,
    );
...
test('picker ignores stale selections, keeps blob URLs local, and cleans up on replacement, reset, and unmount', async () => {
  ...
  const restore = replaceGlobals({
    ...
    createImageBitmap: () =>
      new Promise((resolve) => {
        decodes.push(() => resolve({ width: 10, height: 10, close: () => {} }));
      }),
  });
  ...
  Object.defineProperty(dom.window.HTMLCanvasElement.prototype, 'toBlob', {
    value: (callback: BlobCallback) => callback(new Blob(['jpeg'], { type: PHOTO_OUTPUT_TYPE })),
  });
  ...
  const select = async (name = 'photo.png', type = 'image/png'): Promise<void> => {
    ...
    Object.defineProperty(input, 'files', {
      value: [new File(['fixture'], name, { type })],
      configurable: true,
    });
    await act(async () => {
