...
test('photo preparation blocks publication; failed uploads retain the draft for retry, then saved URLs and removal publish correctly', async () => {
  ...
  const firstSave = new Promise<SiteContent>((_resolve, reject) => { rejectFirstSave = reject; });
  const onSave = async (content: SiteContent, photo?: Blob): Promise<SiteContent> => {
    ...
    return photo ? { ...content, profile: { ...content.profile, photo: { path: savedUrl, alt: content.profile.photo!.alt } } } : content;
  };
  const host = dom.window.document.getElementById('test')!;
  const { createRoot } = await import('react-dom/client');
  const root = createRoot(host);
  ...
  const saveButton = (): HTMLButtonElement => host.querySelector<HTMLButtonElement>('button[type=submit]')!;
  try {
    await act(async () => { root.render(createElement(ContentEditor, { initial, onSave, onDirty: (value) => { dirtyCalls.push(value); } })); });
    ...
    await submit();
    assert.equal(saves.length, 0, 'Native validation must block submission while preparation is pending.');
    await act(async () => { host.querySelector('form')!.dispatchEvent(new dom.window.Event('submit', { bubbles: true, cancelable: true })); });
    assert.equal(saves.length, 0, 'The submit handler must also guard against bypassing native validation.');

    await act(async () => { finishDecode?.(); await decoded; });
    ...
    const first = saves[0]!;
    assert.equal(first.photo, prepared, 'The save callback receives the prepared Blob, not the original File.');
    ...
    assert.equal(upload.matches(':disabled'), true, 'The outer fieldset disables the file control during publication.');

    await act(async () => { rejectFirstSave?.(new Error('Upload failed. Please retry.')); });
    assert.match(host.querySelector('[role=alert]')?.textContent ?? '', /Upload failed\. Please retry\./);
    assert.equal(saveButton().disabled, false);
    ...
    assert.deepEqual(saves[1]!.content, first.content, 'Retry preserves the entire authored draft.');
    ...
    assert.equal([...host.querySelectorAll('button')].some((button) => button.textContent === 'Revert upload'), false);
    ...
    assert.equal(saves[2]!.content.profile.photo?.path, savedUrl, 'Later publication uses the returned HTTPS URL.');
    const remove = [...host.querySelectorAll('button')].find((button) => button.textContent === 'Remove photo');
    assert.ok(remove);
