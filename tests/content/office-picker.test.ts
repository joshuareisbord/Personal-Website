...
const savedOffice: WorkOffice = {
  address: '81 Existing Street, Kingston',
  latitude: 44.232,
  longitude: -76.487,
};
...
function feature(housenumber = '125', properties: Record<string, unknown> = {}) {
  return {
    type: 'Feature',
    geometry: { type: 'Point', coordinates: [-76.49, 44.24] },
    properties: {
      housenumber,
...
test('Photon parser returns longitude/latitude correctly and excludes incomplete addresses and other countries', () => {
  assert.deepEqual(
    parseOfficeResults(
      {
        features: [
          feature(' 125 ', { name: ' Office ' }),
          feature('20', { countrycode: 'US' }),
          feature('21', { street: undefined }),
          feature('22', { housenumber: undefined }),
        ],
      },
      'CA',
    ),
    [
      {
        address: 'Office, 125 Princess Street, Kingston, Ontario, K7L 1A1, Canada',
        latitude: 44.24,
        longitude: -76.49,
      },
    ],
...
test('Photon parser rejects malformed geometry, invalid coordinates, and unbounded response data', () => {
  ...
  for (const payload of [
    ...
    {},
    { features: 'wrong' },
    { features: [feature('1', { countrycode: undefined })] },
    { features: [feature('1', { street: 'x'.repeat(501) })] },
    { features: Array.from({ length: 51 }, () => feature()) },
  ]) {
...
test('Photon parser limits suggestions and omits assembled addresses over the publication limit', () => {
  const results = parseOfficeResults(
    {
      features: [
        feature('1', { name: 'n'.repeat(300), street: 's'.repeat(300) }),
        ...Array.from({ length: 8 }, (_, index) => feature(String(index + 10))),
      ],
    },
    'CA',
  );
...
async function mountPicker(context: TestContext) {
  ...
  const fetchMock: typeof fetch = (input, options) =>
  ...
  function Harness(): ReactElement {
    ...
    }>({ location: cityLabel, place: { ...city, office: savedOffice } });
  ...
  const respond = async (index: number, payload: unknown, status = 200): Promise<void> => {
    ...
    await act(async () => {
      searches[index]!.resolve(new Response(JSON.stringify(payload), { status }));
    });
  };
  return {
    host,
...
test('typing never looks up addresses; explicit search and result selection preserve the city, and removal restores its pin', async (context) => {
  ...
  const result = view.host.querySelector<HTMLButtonElement>(
    '[aria-label="Matching office addresses"] button',
  );
  assert.ok(result);
    ...
    [
      {
        ...
        place: {
          ...city,
          office: {
            address: '125 Princess Street, Kingston, Ontario, K7L 1A1, Canada',
            longitude: -76.49,
            latitude: 44.24,
          },
        },
...
test('query edits abort stale searches and late responses cannot replace newer results', async (context) => {
  ...
  const currentResults = view.host.querySelector(
  ...
  assert.equal(
    view.host.querySelector('[aria-label="Matching office addresses"]')!.textContent,
    currentResults,
  );
...
test('stale failures cannot overwrite a new search, and unmount cancels pending work', async (context) => {
  const view = await mountPicker(context);
  await view.type('701 Old Failure');
  await view.search();
  await view.type('702 Pending Current');
  await view.search();
