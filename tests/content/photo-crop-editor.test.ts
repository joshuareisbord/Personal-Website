...
const hook = registerHooks({
  load(url, context, nextLoad) {
    if (url === cropperUrl) return { format: 'module', shortCircuit: true, source: `
...
async function mountEditor(context: TestContext) {
  const dom = new JSDOM('<div id="test"></div>', { url: 'https://example.com', pretendToBeVisual: true });
  const initial = parseContent({ ...initialContent, profile: { ...initialContent.profile, experience: [],
  ...
  Object.defineProperty(dom.window.HTMLCanvasElement.prototype, 'toBlob', { value: (callback: BlobCallback) => callback(prepared) });
  ...
  const onSave = async (content: SiteContent, photo?: Blob): Promise<SiteContent> => {
    ...
    if (nextFailure) { nextFailure = false; throw new Error('Fixture upload failed. Retry publication.'); }
    return parseContent(photo ? { ...content, profile: { ...content.profile,
      photo: { ...content.profile.photo, path: uploadedPath } } } : content);
  };
  await act(async () => root.render(createElement(ContentEditor, { initial, onSave, onDirty: (value) => dirty.push(value) })));
  const field = (label: string): HTMLInputElement => {
    const element = [...host.querySelectorAll('label')].find((entry) => entry.textContent === label);
  ...
  const hasButton = (label: string): boolean => [...host.querySelectorAll('button')].some((button) => button.textContent === label);
  ...
  const selectUpload = async (): Promise<void> => {
    ...
    Object.defineProperty(input, 'files', { value: [new File(['source PNG fixture'], 'portrait.png', { type: 'image/png' })], configurable: true });
    await act(async () => input.dispatchEvent(new dom.window.Event('change', { bubbles: true })));
    assert.match(host.textContent!, /Photo ready/);
  };
  const submit = async (): Promise<void> => { await act(async () => host.querySelector('form')!.requestSubmit()); };
  return { initial, host, dirty, saves, prepared, revoked, field, hasButton, click, type, selectUpload, submit,
    failNextSave: () => { nextFailure = true; } };
}
...
test('existing crop survives unrelated edits, publication, and subsequent saves', async (context) => {
  ...
  assert.deepEqual(view.saves[0]!.content.profile.photo, { path: originalPath, alt: 'Original portrait', crop: originalCrop });
  assert.equal(view.saves[0]!.photo, undefined);
  assert.deepEqual(parseContent(JSON.parse(serializeContent(view.saves[0]!.content))), view.saves[0]!.content);
  assert.equal(view.dirty.at(-1), false);
  await view.type('Photo description', 'Updated portrait description');
  await view.submit();
  assert.deepEqual(view.saves[1]!.content.profile.photo, { path: originalPath, alt: 'Updated portrait description', crop: originalCrop });
  assert.equal(view.saves[1]!.content.profile.name, 'Edited owner');
  assert.deepEqual(view.initial.profile.photo?.crop, originalCrop, 'Editing must not mutate the published input.');
});
...
test('resetting a crop makes the draft dirty and publishes without stale crop metadata', async (context) => {
  ...
  await view.submit();
  assert.deepEqual(view.saves[0]!.content.profile.photo, { path: originalPath, alt: 'Original portrait' });
  assert.equal(Object.hasOwn(view.saves[0]!.content.profile.photo!, 'crop'), false);
  assert.equal(view.dirty.at(-1), false);
  await view.type('Name', 'Edit after reset');
  await view.submit();
  assert.equal(Object.hasOwn(view.saves[1]!.content.profile.photo!, 'crop'), false, 'A later save must not resurrect the original crop.');
});
...
test('changing a photo link clears its crop while preserving the description and other edits', async (context) => {
  ...
  assert.equal(view.dirty.at(-1), true);
  await view.submit();
  assert.deepEqual(view.saves[0]!.content.profile.photo, { path: 'https://example.com/replacement.jpg', alt: 'Original portrait' });
...
test('reverting an upload after changing the link does not attach the previous photo crop to the new source', async (context) => {
  ...
  await view.submit();
  assert.deepEqual(view.saves[0]!.content.profile.photo, { path: replacement, alt: 'Original portrait' });
  assert.equal(view.saves[0]!.photo, undefined);
});
...
test('a crop applied to a prepared upload survives failed save, retry, returned URL, and later publication', async (context) => {
  ...
  assert.ok(view.host.querySelector('[aria-label="Crop geometry fixture"]'), 'This lifecycle test must use the fixture, not real crop geometry.');
  ...
  await view.submit();
  assert.equal(view.saves[2]!.photo, undefined, 'Successful publication consumes the pending Blob.');
  assert.deepEqual(view.saves[2]!.content.profile.photo, { path: uploadedPath, alt: 'Published cropped portrait', crop: appliedCrop });
});
