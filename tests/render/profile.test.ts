...
test('output permits HTTPS profile images while restricting executable and CSS references', () => {
  assert.doesNotThrow(() => validateStaticHtml('<p>Built reporting with Firestore and api.linkedin.com.</p>'));
  ...
  assert.throws(() => validateStaticCss('a { background: url("https://firebasestorage.googleapis.com/photo"); }'));
  assert.deepEqual(validateStaticCss('a { background: url("/profile/photo.webp"); }'), ['/profile/photo.webp']);
});

test('output permits only local Vite module scripts, rejecting inline and remote execution', () => {
  assert.deepEqual(validateStaticHtml('<script type="module" crossorigin src="/assets/main-Ab_12.js"></script>'), ['/assets/main-Ab_12.js']);
  for (const script of [
  ...
  assert.deepEqual(validateStaticHtml('<link rel="stylesheet" href="/assets/main.css"><img src="/profile/photo.png"><a href="https://www.linkedin.com/in/fixture">Profile</a>'), ['/assets/main.css', '/profile/photo.png']);
});

test('React/Vite builds prerender populated profile HTML and local hydration assets', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    await writeFile(join(root, 'public/profile/fixture.webp'), Buffer.from('UklGRiIAAABXRUJQVlA4IBYAAAAwAQCdASoBAAEADsD+JaQAA3AAAAAA', 'base64'));
    await writeFile(join(root, 'src/data/profile.json'), JSON.stringify({
      schemaVersion: 1, name: "Fixture O'Neil", photo: { path: '/profile/fixture.webp', alt: 'Fixture portrait' },
      experience: [
        { company: 'Example & Company', title: 'Engineer', startDate: '2024-03', endDate: null,
          description: 'Built reporting with Firestore.\nSecond line with <b>literal markup</b>.' },
        { company: 'Earlier Company', title: 'Developer', startDate: '2021', endDate: '2023', location: 'Remote' },
      ],
    }));
    const credential = 'fixture-private-credential-must-never-be-published';
    const options = { cwd: root, timeout: 45_000, env: { ...process.env, CI: 'true', FIREBASE_SERVICE_ACCOUNT: credential } };
    const vite = join(project, 'node_modules/vite/bin/vite.js');
    await exec(process.execPath, [vite, 'build'], options);
    await exec(process.execPath, [vite, 'build', '--ssr', 'src/entry-server.tsx', '--outDir', '.prerender'], options);
    ...
    assert.equal(images.length, 1, 'The populated profile must render exactly one image, with no logo images.');
    ...
    assert.doesNotMatch(html, /<!--app-html-->|https:\/\/(?:api\.linkedin\.com|firestore\.googleapis\.com|firebasestorage\.googleapis\.com)/);
    ...
    const fontReferences = (await Promise.all(stylesheets.map(async (reference) => {
    ...
    assert.ok(fontReferences.some((reference) => /^\/assets\/barlow-.*\.woff2$/.test(reference)), 'Barlow must be referenced as a local WOFF2 build asset.');
    assert.ok(fontReferences.some((reference) => /^\/assets\/ibm-plex-mono-.*\.woff2$/.test(reference)), 'IBM Plex Mono must be referenced as a local WOFF2 build asset.');
