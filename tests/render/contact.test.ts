    ...
    for (const phone of ['', initialContent.site.phone]) {
      const content = parseContent({
        ...
        site: {
          ...
          email,
          phone,
          phoneHref: phone ? initialContent.site.phoneHref : '',
        },
      });
      ...
      try {
        ...
        assert.equal(
          contact.querySelectorAll('a[href=""], a[href="mailto:"], a[href="tel:"]').length,
          0,
        );
...
test('contact social links sit below their label and use Material SVG icons without footer duplicates', () => {
  const dom = new JSDOM(
    renderToStaticMarkup(createElement(Home, { content: initialContent, year: 2026 })),
  );
  try {
    ...
    assert.equal(
      dom.window.document
        .querySelector('#social-heading')
        ?.nextElementSibling?.querySelectorAll('a').length,
      2,
    );
    assert.equal(
      dom.window.document.querySelectorAll('footer #social-links, footer svg').length,
      0,
    );
    for (const link of links) {
      assert.ok(link.closest('#contact'));
      assert.equal(
        link.querySelectorAll('svg[fill="currentColor"][aria-hidden="true"][focusable="false"]')
          .length,
        2,
      );
