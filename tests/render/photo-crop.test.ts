...
test('photo crops round-trip without changing legacy or absent photos', () => {
  for (const photo of [null, { path: '/profile/portrait.webp', alt: 'Portrait' },
    { path: '/profile/portrait.webp', alt: 'Portrait', crop: { x: 25, y: 10, width: 50, height: 80 } },
  ]) {
...
test('photo crop bounds allow rounding noise but reject malformed and overflowing crops', () => {
  ...
  assert.ok(photoCropSchema.safeParse({ x: 0.1, y: 20, width: 99.90000000001, height: 80.00000000001 }).success);
  assert.ok(photoCropSchema.safeParse({ x: 99, y: 99, width: 1, height: 1 }).success);
  for (const crop of [null, [], {}, { x: 0, y: 0, width: 100 }, { ...full, extra: true },
    ...['x', 'y', 'width', 'height'].flatMap((key) => [NaN, Infinity, -Infinity, '1', null, -0.01].map((value) => ({ ...full, [key]: value }))),
    { ...full, width: 0 }, { ...full, height: 0 }, { ...full, width: 100.001 }, { ...full, height: 100.001 },
    { ...full, x: 0.0001 }, { ...full, y: 0.0001 },
  ]) {
    assert.equal(photoCropSchema.safeParse(crop).success, false);
    assert.throws(() => parseContent({ ...initialContent, profile: { ...initialContent.profile,
...
test('crop layout maps the selected source rectangle exactly onto the photo frame', () => {
  for (const crop of [
    ...
    const near = (value: number, expected: number): void => assert.ok(Math.abs(value - expected) < 1e-9);
...
test('shared photos render accessible cover defaults and explicit percentage crops', () => {
  for (const crop of [undefined, { x: 25, y: 10, width: 50, height: 80 }]) {
    const document = new JSDOM(renderToStaticMarkup(createElement(ProfilePhoto, {
      src: '/profile/portrait.webp', alt: 'Portrait', crop, className: 'grayscale', loading: 'lazy',
    }))).window.document;
    ...
    for (const name of ['relative', 'aspect-[4/5]', 'overflow-hidden', 'grayscale']) assert.ok(img.parentElement!.classList.contains(name));
    if (crop) {
      ...
      for (const name of ['object-cover', 'w-full', 'h-full']) assert.ok(img.classList.contains(name));
...
test('About uses the shared crop and fills its existing left column', () => {
  const content = { ...initialContent, profile: { ...initialContent.profile,
    photo: { path: '/profile/portrait.webp', alt: 'Portrait', crop: { x: 25, y: 10, width: 50, height: 80 } } } };
  const document = new JSDOM(renderToStaticMarkup(createElement(Home, { content, year: 2026 }))).window.document;
  const about = document.querySelector('#about')!;
  const figure = about.querySelector('figure')!;
  assert.ok(about.classList.contains('lg:grid-cols-[1fr_2fr]'));
  for (const name of ['w-full', 'max-w-md', 'lg:max-w-none']) assert.ok(figure.classList.contains(name));
