...
describe('CMS Firestore access rules', { skip: !emulatorHost, concurrency: false }, () => {
  ...
  before(async () => {
    ...
    assert.ok(
      ...
      'Rules tests require a local emulator.',
    );
  ...
  beforeEach(async () => {
    ...
    await environment.withSecurityRulesDisabled(async (context) => {
      ...
      await setDoc(doc(db, 'website/content'), {
        payload: '{}',
        revision: 1,
  ...
  const authenticated = (
    ...
    email: string,
    verified = true,
    provider: 'google.com' | 'password' | 'custom' | 'anonymous' = 'google.com',
  ) =>
    environment
      .authenticatedContext(uid, {
        email,
        email_verified: verified,
        firebase: { sign_in_provider: provider, identities: { 'google.com': [uid] } },
      })
      .firestore();
  ...
  test('public can get only content, with no collection listing or writes', async () => {
    ...
    await assertFails(
      updateDoc(doc(db, 'website/content'), {
        payload: 'public edit',
        revision: 2,
  ...
  test('verified Google owners can edit and case-normalized emails identify the same owner', async () => {
    for (const [uid, email, revision] of [
      ['alice', 'Alice@Example.Test', 2],
      ['bob', 'bob@example.test', 3],
    ] as const) {
      const db = authenticated(uid, email);
      await assertSucceeds(
        setDoc(doc(db, 'website/content'), {
          payload: '{"bio":"Updated"}',
          revision,
          updatedAt: serverTimestamp(),
          updatedBy: uid,
  ...
  test('unverified or non-Google sign-in cannot use owner privileges even with a linked Google identity', async () => {
    for (const [verified, provider] of [
      [false, 'google.com'],
      [true, 'password'],
      [true, 'custom'],
      [true, 'anonymous'],
  ...
  test('owners add/delete other owners, while self-removal and invalid owner documents are denied', async () => {
    ...
    await assertFails(
      setDoc(doc(db, 'websiteOwners/carol@example.test'), { enabled: true, role: 'admin' }),
    );
  ...
  test('revocation blocks subsequent writes, listings and restoration of access', async () => {
    ...
    await assertFails(
      setDoc(doc(bob, 'website/content'), {
        payload: '{}',
        revision: 2,
        updatedAt: serverTimestamp(),
        updatedBy: 'bob',
  ...
  test('content creation starts at revision 1 and simultaneous stale transactions cannot overwrite a winner', async () => {
    ...
    const save = (payload: string): Promise<number> =>
      runTransaction(db, async (transaction) => {
        const snapshot = await transaction.get(reference);
        if (snapshot.data()?.['revision'] !== 1)
          throw new Error('Content changed. Reload before saving.');
  ...
  test('a batch cannot self-enroll and publish content together', async () => {
    ...
    batch.set(doc(db, 'website/content'), {
      payload: '{}',
      revision: 2,
      updatedAt: serverTimestamp(),
      updatedBy: 'outsider',
  ...
  test('legacy project access is preserved without granting CMS or unrelated collection access', async () => {
    ...
    for (const uid of ['0ePINre65yeCTJG7EcEA8oCweOT2', 'BgQIemYzEjhBvqTSS9HHQ20lrWR2']) {
      ...
      await assertSucceeds(
        setDoc(doc(legacyDb, 'projects/example/details/metadata'), { version: 1 }),
      );
      await assertFails(
        setDoc(doc(legacyDb, 'websiteOwners/legacy@example.test'), { enabled: true }),
      );
      await assertFails(getDocs(collection(legacyDb, 'websiteOwners')));
      await assertFails(
        setDoc(doc(legacyDb, 'website/content'), {
          payload: '{}',
          revision: 2,
          updatedAt: serverTimestamp(),
          updatedBy: uid,
    ...
    await assertFails(
      setDoc(doc(authenticated('outsider', 'outsider@example.test'), 'projects/example'), {
        title: 'Denied',
      }),
    );
  });
