...
describe(
  'Owner photo Storage rules',
  { skip: !firestoreHost || !storageHost, concurrency: false },
  () => {
    ...
    beforeEach(async () => {
      ...
      await environment.withSecurityRulesDisabled(async (context) => {
        await setDoc(doc(context.firestore(), 'websiteOwners/alice@example.test'), {
          enabled: true,
        });
      });
    });
    ...
    test('approved Google owners upload; signed-out visitors can read but cannot list', async () => {
      ...
      assert.equal(
        (await assertSucceeds(getBytes(ref(publicStorage, name)))).byteLength,
        jpeg.byteLength,
      );
    ...
    test('public, unapproved, unverified, and non-Google accounts cannot upload', async () => {
      for (const context of [
        ...
        user('outsider', 'other@example.test'),
        user('alice', 'alice@example.test', false),
        user('alice', 'alice@example.test', true, 'password'),
      ]) {
      ...
      await assertFails(
        uploadBytes(ref(user().storage(), 'website-profile/loose.jpg'), jpeg, metadata),
      );
    });
    ...
    test('invalid types, empty files, oversized files and names are rejected', async () => {
      ...
      await assertFails(
        uploadBytes(ref(storage, 'website-profile/alice/photo.png'), jpeg, metadata),
      );
    ...
    test('replacements use new objects; nobody can overwrite or delete published photos', async () => {
      ...
      await assertFails(
        deleteObject(ref(environment.unauthenticatedContext().storage(), target.fullPath)),
      );
    });

    test('revocation and malformed owner records immediately block new uploads', async () => {
      await environment.withSecurityRulesDisabled(async (context) => {
        await deleteDoc(doc(context.firestore(), 'websiteOwners/alice@example.test'));
      });
    ...
    test('legacy paths retain existing authenticated access without opening the upload prefix', async () => {
      ...
      await assertFails(
        uploadBytes(
          ref(environment.unauthenticatedContext().storage(), 'legacy.png'),
          jpeg,
          metadata,
        ),
      );
