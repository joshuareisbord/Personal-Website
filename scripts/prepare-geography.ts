...
assert(
  [...args].every((arg) => ['--check', '--offline', '--locations-only'].includes(arg)),
  'Use --check, --offline, and/or --locations-only',
);
...
async function packageVersion(name: string): Promise<string> {
  const pkg = JSON.parse(await readFile(join(packageRoot(name), 'package.json'), 'utf8')) as {
    version: string;
  };
  return pkg.version;
}
...
function restoreCollapsedRings(weighted: Boundaries, candidate: Boundaries): boolean {
  ...
  for (const [index, country] of countries.features.entries()) {
    ...
    const rings = polygons.flatMap((polygon) =>
      polygon.map((ring, index) => ({ ring, interior: index > 0 })),
    );
    ...
    for (const [ringIndex, { ring, interior }] of rings.entries()) {
      ...
      if (validWinding && new Set(ring.map((point) => `${point[0]},${point[1]}`)).size >= 3)
        continue;
...
function validateBoundaries(boundaries: Boundaries, expectedLines: number): void {
  ...
  for (const [index, line] of lines.geometry.coordinates.entries()) {
    ...
    assert(
      new Set(line.map((point) => `${point[0]},${point[1]}`)).size >= 2,
      `Collapsed admin-1 line ${index}`,
    );
  }
...
async function prepareBoundaries(): Promise<{ boundaries: Boundaries; stats: object }> {
  ...
  const countries = JSON.parse(countryBytes!.toString()) as FeatureCollection<
    Polygon | MultiPolygon,
    SourceProperties
  >;
  const admin1 = JSON.parse(adminBytes!.toString()) as FeatureCollection<
    ...
    features: countries.features
      .map((country) => {
        ...
        return {
          ...
          id: String(country.properties.ADM0_A3),
          geometry: country.geometry,
          properties: {
            name: String(country.properties.NAME_EN),
  ...
  for (const entry of admin1.features) {
    ...
    for (const part of parts) {
      ...
      if (new Set(part.map((point) => `${point[0]},${point[1]}`)).size < 2) {
        zeroLengthParts++;
        continue;
      }
  ...
  const meshZeroLengthParts = stitchedLines.coordinates.filter(
    (line) => new Set(line.map((point) => `${point[0]},${point[1]}`)).size < 2,
  ).length;
  stitchedLines.coordinates = stitchedLines.coordinates.filter(
    (line) => new Set(line.map((point) => `${point[0]},${point[1]}`)).size >= 2,
  );
  ...
  assert.equal(
    sourceSegments.size,
    stitchedSegments.size,
    'Stitching changed drawable segment count',
  );
  assert(
    [...sourceSegments].every((segment) => stitchedSegments.has(segment)),
    'Stitching dropped source linework',
  );
  ...
  return {
    boundaries,
    stats: {
      countries: cleaned.features.length,
      admin1SourceFeatures: admin1.features.length,
      admin1NullGeometries: nullFeatures,
      admin1ZeroLengthParts: zeroLengthParts,
      admin1MeshZeroLengthParts: meshZeroLengthParts,
      admin1UniqueSourceSegments: sourceSegments.size,
      admin1SourceLineParts: lines.coordinates.length + zeroLengthParts,
      admin1LineParts: stitchedLines.coordinates.length,
      admin1FeaturesByCountry: Object.fromEntries(
        Object.entries(featuresByCountry).sort(([a], [b]) => compare(a, b)),
      ),
      ...
      sphericalTriangleMinWeight: MIN_WEIGHT,
...
function prepareLocations(): {
  ...
  locations: Map<string, CountryLocations>;
  stats: object;
} {
  const countries = Country.getAllCountries()
    .map(({ isoCode: code, name }) => ({ code, name }))
  ...
  for (const city of City.getAllCities()) {
    ...
    assert(
      target && target.regions.some((region) => region.code === city.stateCode),
      `Orphan city: ${city.name}`,
    );
    ...
    if (previous) {
      assert.equal(previous, identity, 'City ID hash collision');
      duplicates++;
      continue;
    }
    ids.set(id, identity);
  ...
  for (const data of locations.values()) {
    ...
    data.cities.sort(
      (a, b) =>
        compare(a.name, b.name) || compare(a.regionCode, b.regionCode) || compare(a.id, b.id),
    );
  ...
  for (const code of ['AK', 'HI', 'CA', 'NY', 'DC'])
    assert(locations.get('US')!.regions.some((region) => region.code === code));
  ...
  return {
    ...
    stats: {
      ...
      regions,
      cities: ids.size,
      exactDuplicateCities: duplicates,
      regionsWithoutCities,
...
async function main(): Promise<void> {
  assert.equal(
    await packageVersion('country-state-city'),
    '3.2.1',
    'Review data changes before changing the pinned package version',
  );
  const { countries, locations, stats: locationStats } = prepareLocations();
  const output = new Map<string, Buffer>();
  const json = (name: string, value: unknown): void => {
    output.set(name, Buffer.from(`${JSON.stringify(value)}\n`));
  };
  ...
  if (args.has('--locations-only')) {
    for (const [name, bytes] of output) {
      ...
      if (args.has('--check'))
        assert.deepEqual(await readFile(path), bytes, `Generated asset is stale: ${name}`);
      else {
    ...
    console.log(
      JSON.stringify(
        { mode: args.has('--check') ? 'verified' : 'generated', locations: locationStats },
        null,
        2,
      ),
    );
  ...
  console.log(
    `Boundary payload: ${boundaryBytes.length} bytes, ${gzipSync(boundaryBytes, { level: 9 }).length} bytes gzip`,
  );
  assert(
    gzipSync(boundaryBytes, { level: 9 }).length <= 1_000_000,
    'Compressed boundary payload exceeds budget',
  );
  output.set(
    'LICENSE.country-state-city.txt',
    await readFile(join(packageRoot('country-state-city'), 'LICENSE')),
  );
  output.set(
    'ATTRIBUTION.md',
    Buffer.from(
  ...
  for (const name of [
    ...
    versions[name] = await packageVersion(name);
  const dataHashes: Record<string, string> = {};
  for (const name of ['country', 'state', 'city']) {
    dataHashes[`${name}.json`] = sha256(
      await readFile(join(packageRoot('country-state-city'), `lib/assets/${name}.json`)),
    );
  }
  json('manifest.json', {
    schemaVersion: 1,
    naturalEarth: { tag: 'v5.1.2', revision: REVISION, sources: SOURCES },
    packages: versions,
    countryStateCitySourceSha256: dataHashes,
  ...
  console.log(
    JSON.stringify(
      {
        mode: args.has('--check') ? 'verified' : 'generated',
        files: output.size,
        boundaries: artifacts['boundaries.topo.json'],
        countries: artifacts['countries.json'],
        locations: locationStats,
      },
      null,
      2,
