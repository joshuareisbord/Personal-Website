...
function validateReference(value: string, allowHttpsImage = false): string | null {
  ...
  if (!value || /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i.test(value) || /[\\\s<>]|&(?:#|[a-z]+;)/i.test(value)) {
...
export function validateStaticHtml(html: string): string[] {
  ...
  const withoutScripts = markup.replace(/<script\b((?:[^>"']|"[^"]*"|'[^']*')*)>([\s\S]*?)<\/script\s*>/gi, (_match: string, rawAttributes: string, body: string) => {
    const attrs = attributes(rawAttributes);
    const src = attrs.get('src') ?? '';
    if (attrs.get('type') !== 'module' || !modulePath.test(src) || body.trim() ||
  ...
  if (/<(?:script|iframe|object|embed|base)\b/i.test(withoutScripts)) throw new Error('Unexpected executable HTML in public output.');
  for (const match of withoutScripts.matchAll(tagPattern)) {
    const tag = (match[1] ?? '').toLowerCase();
    const attrs = attributes(match[2] ?? '');
    for (const [key, value] of attrs) {
      if (['src', 'href', 'action', 'formaction'].includes(key) &&
        (/^\s*(?:javascript|vbscript|data):/i.test(value) && !(key === 'src' && tag === 'img'))) {
    ...
    if (tag === 'img' || tag === 'source') {
      ...
      if (attrs.has('srcset')) {
        ...
        if (/data:/i.test(srcset)) throw new Error('Inline srcset is not supported by output verification.');
        for (const candidate of srcset.split(',')) add(candidate.trim().split(/\s+/)[0] ?? '', true);
      }
    }
    if (tag === 'link' && /(?:^|\s)(?:stylesheet|modulepreload|preload|icon)(?:\s|$)/i.test(attrs.get('rel') ?? '')) add(attrs.get('href') ?? '');
    if (attrs.has('style')) references.push(...validateStaticCss(attrs.get('style') ?? ''));
  }
  for (const match of withoutScripts.matchAll(/<style\b[^>]*>([\s\S]*?)<\/style\s*>/gi)) references.push(...validateStaticCss(match[1] ?? ''));
...
export function validateStaticCss(css: string): string[] {
  ...
  for (const match of clean.matchAll(/(?:url\(\s*|@import\s+)(?:"([^"]*)"|'([^']*)'|([^\s"')]+))/gi)) {
