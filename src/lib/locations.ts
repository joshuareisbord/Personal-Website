...
export const countriesSchema = z
  .array(z.object({ code: z.string().regex(/^[A-Z]{2}$/), name }))
  .max(300);
  ...
  return data.cities
    .filter(
      (city) =>
        ...
        terms.every((term) => normalize(city.name).includes(term)),
    )
    .slice(0, 12);
}
...
export function selectCity(
  ...
  data: LocationData,
  city: City,
): { location: string; place: WorkPlace } {
  ...
  return {
    ...
    place: {
      ...
      ...(city.regionCode ? { regionCode: city.regionCode } : {}),
      city: city.name,
    },
  };
}
