...
const httpsUrl = z.string().max(2000).refine(isPublicHttpsUrl, 'Use an HTTPS link.');
const siteSchema = z
  .object({
    tagline: text,
      ...
      .pipe(
        z.union([
          ...
          z
            .string()
            .regex(/^tel:\+?[0-9 ()-]{3,30}$/)
            .refine((value) => value.replace(/\D/g, '').length >= 3, 'Enter a valid phone number.'),
        ]),
      )
      .default(''),
    github: httpsUrl,
  ...
  .strict()
  .refine((site) => Boolean(site.phone) === Boolean(site.phoneHref), {
    message: 'Provide both a phone number and its call link, or leave both blank.',
    path: ['phone'],
  });
...
export function parseContent(value: unknown): SiteContent {
  ...
  if (new TextEncoder().encode(JSON.stringify(content)).length > 200_000)
    throw new Error('Content is too large. Shorten the text and try again.');
  return content;
}
