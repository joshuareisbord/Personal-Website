...
export function validatePhotoInput(file: PhotoMetadata): void {
  if (!PHOTO_INPUT_TYPES.some((type) => type === file.type)) throw new Error('Choose a PNG, JPEG, or WebP image.');
  if (!Number.isSafeInteger(file.size) || file.size <= 0) throw new Error('Choose a non-empty image file.');
...
export function getPhotoDimensions(width: number, height: number): { width: number; height: number } {
  if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width <= 0 || height <= 0
    || width > MAX_PHOTO_INPUT_DIMENSION || height > MAX_PHOTO_INPUT_DIMENSION || width * height > MAX_PHOTO_PIXELS) {
...
function encodeJpeg(canvas: HTMLCanvasElement): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    }, PHOTO_OUTPUT_TYPE, PHOTO_JPEG_QUALITY);
  });
}
...
export async function preparePhoto(file: File): Promise<Blob> {
  ...
  catch { throw new Error('Could not decode this image. Choose a valid PNG, JPEG, or WebP photo.'); }
