...
import { useSignalMotion } from './geography/use-signal-motion';
...
export function SignalStudy(): ReactElement {
  ...
  useSignalMotion(container, lines);
  return (
    <div
      ref={container}
      aria-hidden="true"
      className="relative isolate h-64 overflow-hidden bg-night sm:h-80 lg:h-[25rem]"
    >
      <svg
      ...
      >
        <defs>
          <pattern id="signal-grain" width="7" height="7" patternUnits="userSpaceOnUse">
            <circle cx="1" cy="1" r="0.65" fill="currentColor" />
          </pattern>
          <pattern id="signal-grid" width="100" height="100" patternUnits="userSpaceOnUse">
            <path d="M100 0H0V100" stroke="currentColor" strokeWidth="0.5" />
          </pattern>
          <clipPath id="signal-crop">
            <rect width="1400" height="460" />
          </clipPath>
        </defs>
        <g className="text-chalk" clipPath="url(#signal-crop)">
          ...
          <g
            ref={lines}
            transform="translate(865 240) rotate(-24)"
            stroke="currentColor"
            strokeWidth="0.75"
          >
            {Array.from({ length: 26 }, (_, index) => (
              <ellipse
                ...
                rx={95 + index * 11}
                ry={235 - index * 4.7}
                opacity={0.22 + index / 70}
                transform={`rotate(${index * 3.2})`}
              />
            ))}
          ...
          <path
            d="M260 335h40m-20-20v40M1180 85h24m-12-12v24"
            stroke="currentColor"
            opacity="0.65"
          />
          <circle cx="280" cy="335" r="5" fill="currentColor" />
          <path
            d="M34 54V30h24M1342 30h24v24M34 406v24h24M1342 430h24v-24"
            stroke="currentColor"
            opacity="0.65"
          />
        </g>
